...
// which has had a screen of its own since 23 Sep (Petra: the page was too long,
// and the ledger alone was over a third of it).
//
  ...
  return (
    <div className="lv-enter rounded-[var(--r)] bg-sf px-[18px] pb-1 pt-1.5 text-tx">
      <div className="flex items-center justify-between border-b border-ln py-2.5">
        ...
        <Link href="/money/ledger" className="text-[13px] text-tx3">all entries ›</Link>
      </div>
      {rows.map(({ e }) => (
        <button
          key={e.id}
